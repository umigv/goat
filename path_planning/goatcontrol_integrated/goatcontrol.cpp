...
bool GoatControl::make_reachable_collection()
{
    ...
    {
        GoatControl::position current = open_set.front();
        open_set.pop_front();
...
void GoatControl::add_reachable_adjacent(unsigned int x, unsigned int y)
{
    ...
    GoatControl::position northeast = GoatControl::position(x + 1, y + 1);
    ...
    GoatControl::position east = GoatControl::position(x + 1, y);
    ...
    GoatControl::position southeast = GoatControl::position(x + 1, y - 1);
    ...
    GoatControl::position north = GoatControl::position(x, y + 1);
    ...
    GoatControl::position south = GoatControl::position(x, y - 1);
    ...
    GoatControl::position northwest = GoatControl::position(x - 1, y + 1);
    ...
    GoatControl::position west = GoatControl::position(x - 1, y);
    ...
    GoatControl::position southwest = GoatControl::position(x - 1, y - 1);
...
void GoatControl::backtracker(vector<GoatControl::position> &solution_path)
{
    ...
    {
        solution_path.push(GoatControl::position(x, y));
        GoatControl::position before = backtrack_map[x][y];
        x = before.x;
        y = before.y;
    }
} // backtracker
...
double GoatControl::distance(GoatControl::position a, GoatControl::position b)
{
...
unsigned int GoatControl::min_cost(GoatControl::position p)
{
    unsigned int min = 0;
    unsigned int x = p.x;
    unsigned int y = p.y;
...
void GoatControl::gpsCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    */
    ...
    this->target = GoatControl::position(msg->pose.pose.position.x, msg->pose.pose.position.y);
...
void GoatControl::costmapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
    ...
    */
    ...
    this->start = GoatControl::position(msg->info.origin.orientation.x, msg->info.origin.orientation.y);
